...
#include <omp.h> // Agregar librería para paralelizar con OpenMP 
...
bool relax_until_converge(int max_iterations = 350000) {  // Aumentado para Re=5
    ...
    do {
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp for schedule(dynamic,20) reduction(max:max_diff_u, max_diff_w)
            for (int i = 1; i < Nxmax; ++i) {
                for (int j = 1; j < Nymax; ++j) {
                    if (is_inside_beam(i, j)) continue;
...
void export_streamfunction(double reynolds) {
    ...
    std::string filename = "Datos/streamfunction_Re_dynamic" + reynolds_str + ".dat";
...
void export_vorticity(double reynolds) {
    std::string reynolds_str = format_reynolds(reynolds);
    std::string filename = "Datos/vorticity_Re_dynamic" + reynolds_str + ".dat";
    std::ofstream file(filename);
...
void export_velocity_field(double reynolds) {
    std::string reynolds_str = format_reynolds(reynolds);
    std::string filename = "Datos/velocity_field_Re_dynamic" + reynolds_str + ".dat";
    std::ofstream file(filename);
...
int main() {
    ...
    for (double re : reynolds_values) {
        ...
        std::cout << "    - Datos/streamfunction_Re_dynamic" << re_str << ".dat" << std::endl;
        std::cout << "    - Datos/vorticity_Re_dynamic" << re_str << ".dat" << std::endl;
        std::cout << "    - Datos/velocity_field_Re_dynamic" << re_str << ".dat" << std::endl;
    }
