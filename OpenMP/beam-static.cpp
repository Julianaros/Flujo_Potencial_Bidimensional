...
bool relax_until_converge(int max_iterations = 350000) {  // Aumentado para Re=5
    ...
    do {
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp for schedule(static) reduction(max:max_diff_u, max_diff_w)
            for (int i = 1; i < Nxmax; ++i) {
                for (int j = 1; j < Nymax; ++j) {
                    if (is_inside_beam(i, j)) continue;
...
int main() {
    ...
    for (double re : reynolds_values) {
        ...
        std::cout << "    - Datos/streamfunction_Re_static" << re_str << ".dat" << std::endl;
        std::cout << "    - Datos/vorticity_Re_static" << re_str << ".dat" << std::endl;
        std::cout << "    - Datos/velocity_field_Re_static" << re_str << ".dat" << std::endl;
    }
